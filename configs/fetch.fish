    ...
    switch $distro
        ...
        case archlinux
            for arch in armv7 aarch64 x86_64
                if test "$arch" = x86_64
                    set url https://gitlab.archlinux.org/archlinux/packaging/packages/linux/-/raw/main/config.$arch
                else
        ...
        case debian
            ...
            for arch in $deb_arches
                set package_version_signed 7.1
                ...
                else if test $package_version_signed = 7.1
                    set kernel_version_signed (string replace - '~' $package_version_signed).1-1~exp1
                else
